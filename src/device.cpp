...
Device::Device(Device&& other) noexcept
    : physical_handle(std::move(other.physical_handle)),
      logical_handle(std::move(other.logical_handle)),
      suitability(std::move(other.suitability)),
...
void Device::create(const Instance& instance) {
...
void Device::initLogical(const Instance& instance) {
    ...
    std::cout << "suitability.queue_fam_indexes.size(): "
              << suitability.queue_fam_indexes.size() << std::endl;
    std::map<std::string, uint32_t>::iterator it;
    uint32_t last_queue_index{0};
    uint32_t queue_count{1};
    for (it = suitability.queue_fam_indexes.begin();
         it != suitability.queue_fam_indexes.end(); it++) {
        VkDeviceQueueCreateInfo queue_information{};
        queue_information.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
        std::cout << "queue_information.queueFamilyIndex: " << it->second
                  << std::endl;
    ...
    suitability.vulkan12_features.sType =
        VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_2_FEATURES;
    suitability.vulkan12_features.bufferDeviceAddress = VK_TRUE;
    suitability.vulkan12_features.pNext = nullptr;
    suitability.vulkan12_features.descriptorIndexing = VK_TRUE;

    VkPhysicalDeviceDynamicRenderingFeaturesKHR
        dynamic_rendering_info{};  // Zero initialize
    dynamic_rendering_info.sType =
        VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DYNAMIC_RENDERING_FEATURES_KHR;
    dynamic_rendering_info.pNext = &suitability.vulkan12_features;
    ...
    // Enable the feature
    suitability.buffer_device_address_features.bufferDeviceAddress = VK_TRUE;

    if (instance.enable_validation) {
        device_information.enabledLayerCount =
            static_cast<uint32_t>(instance.validation_layers.size());
        device_information.ppEnabledLayerNames =
            instance.validation_layers.data();
    } else {
    ...
    device_information.ppEnabledExtensionNames =
        required_device_extensions.data();

    vkCheck(vkCreateDevice(physical_handle, &device_information, nullptr,
                           &logical_handle));
...
void Device::initPhysical(const Instance& instance) {
    ...
    vkEnumeratePhysicalDevices(instance.handle, &num_device_found,
                               devices.data());
...
void Device::checkDeviceSuitability(const VkPhysicalDevice& device,
    ...
    // features2.pNext = &suitability.buffer_device_address_features;
    ...
    for (const auto& fam : queue_fams) {
        ...
        suitability.queue_fam_draw_suitable =
        ...
        vkCheck(vkGetPhysicalDeviceSurfaceSupportKHR(
            device, i, instance.surface, &present_support));
        if (present_support) {
...
void Device::checkDeviceExtensions(const VkPhysicalDevice& device,
    ...
    suitability.extension_suitable =
        (required_extensions.size() == found_required);
}

void Device::querySwapChainSupport(const VkPhysicalDevice& device,
                                   const Instance& instance) {

    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, instance.surface,
                                              &suitability.capabilities);
    uint32_t num_formats{0};
    vkGetPhysicalDeviceSurfaceFormatsKHR(device, instance.surface, &num_formats,
                                         nullptr);

    if (num_formats != 0) {
        suitability.formats.resize(num_formats);
        vkGetPhysicalDeviceSurfaceFormatsKHR(
            device, instance.surface, &num_formats, suitability.formats.data());
    }

    uint32_t num_present_modes{0};
    vkGetPhysicalDeviceSurfacePresentModesKHR(device, instance.surface,
                                              &num_present_modes, nullptr);

    if (num_present_modes != 0) {
        suitability.present_modes.resize(num_present_modes);
        vkGetPhysicalDeviceSurfacePresentModesKHR(
            device, instance.surface, &num_present_modes,
            suitability.present_modes.data());
    }
}

void Device::initQueues() {
    vkGetDeviceQueue(logical_handle, suitability.queue_fam_indexes["draw"], 0,
                     &graphics_queue);
    vkGetDeviceQueue(logical_handle, suitability.queue_fam_indexes["present"],
                     0, &present_queue);
