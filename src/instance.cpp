...
namespace Vulkan {
    ...
    void createInstance(Instance& instance)
    {
      // Validation layer check, TODO wrap in  a debug thing
        ...
        vkEnumerateInstanceLayerProperties(&layers, layers_found.data());
        bool found = true && layers;
        for (const char* name : instance.validation_layers) {
            for (const auto& props : layers_found) {
                std::cout << name << " " << props.layerName << std::endl;
                if (strcmp(name, props.layerName) == 0) {
                    std::cout << "found layer" << std::endl;
        ...
        if (found) {
            create_information.enabledLayerCount = static_cast<uint32_t>(instance.validation_layers.size());
            create_information.ppEnabledLayerNames = instance.validation_layers.data();
            extensions.push_back(VK_EXT_DEBUG_UTILS_EXTENSION_NAME);
            create_information.pNext = static_cast<VkDebugUtilsMessengerCreateInfoEXT*>(&instance_debug_info);
        } else {
            create_information.enabledLayerCount = 0;
        ...
        vkCheck(vkCreateInstance(&create_information, nullptr, &instance.handle));
        std::cout << "Created vulkan instance" << std::endl;
        ...
        if (func != nullptr) {
          func(instance.handle, &messenger_info, nullptr, &instance.debug_messenger);
        }
    }
    ...
    void destroyInstance(Instance& instance)
    {
        ...
        if (func != nullptr) {
            func(instance.handle, instance.debug_messenger, nullptr);
        }
    ...
    {
        vkCheck(glfwCreateWindowSurface(instance.handle, window.handle, nullptr, &instance.surface));
    }
